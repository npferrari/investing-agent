...
**2026-07-24 (same day, later) — sleeve/position truth consolidated to one source; two more bugs found tracing it.**

The user asked why a CI light run briefed AAPL as held when "no order was ever submitted" (from this session's perspective). Reconciled: `get_positions()` confirmed a real, tiny ($0.003) fractional-share remainder from an earlier smoke-test BUY/SELL ($50, this session) — genuine live Alpaca state, not a bug. But tracing *how* "held AAPL" and its sleeve reached each consumer surfaced a real, live inconsistency: `execute.py`, `journal.py`, and `main.py` all correctly read the tracked sleeve from `positions_state.json` (falling back to `DEFAULT_SLEEVE` per-call when untracked), but **`briefing.py`'s `_position_line()` never actually read `positions_state` at all** — it still hardcoded `DEFAULT_SLEEVE` unconditionally, a leftover comment-and-code from before `positions_state.py` existed that never got updated when the module was built. The LLM-facing briefing and the risk/journal path had been silently using two different sleeve sources since `positions_state.py` was introduced; they happened to agree only because `DEFAULT_SLEEVE` and the one real tracked sleeve in play were both "MOMENTUM."

Fix: consolidated to one path. `positions_state.reconcile(live_symbols)` is now the single function that resolves sleeve *and* `opened_at` against live reality — it prunes stale tracked entries (symbols no longer held) and logs a visible warning (not a silent per-call fallback) for any live position with no tracked entry, which is exactly what the dust-remnant case is: `record_close()` unconditionally drops the tracked entry on a SELL submission, but Alpaca's fill left a fractional share behind, so Alpaca still reports it held while our bookkeeping already considers it closed. `execute.get_positions_with_sleeves()` is the one function that combines Alpaca's live position list with `reconcile()`, returning plain dicts (`symbol`, `qty`, `market_value`, `avg_entry_price`, `unrealized_pl`, `unrealized_plpc`, `sleeve`, `opened_at`) that `main.py`, `journal.py`, and `briefing.py` all now consume identically — no module re-derives sleeve independently anymore. `briefing._position_line()` now reads the real resolved sleeve, and prefers `positions_state`'s own `opened_at` for "days held" over `journal.position_opened_date()`'s journal-scan inference (demoted to a documented fallback for untracked positions only — itself less reliable than before, since the git-add bug above means the journal has real gaps in its history now).

Two more bugs found only by re-testing live after this fix: (1) `screener.select_candidates()` still did `p.symbol` attribute access, assuming raw Alpaca objects — broke `--mode full` immediately (`AttributeError: 'dict' object has no attribute 'symbol'`) until updated to `p["symbol"]`. (2) The per-holding-always light-run redesign from the entry above had its own latent bug: a single-symbol focused call can still return *two* candidate entries despite "return exactly one, for {symbol} only" — the second a fully degenerate placeholder (empty symbol, zero confidence, empty rationale/thesis). The code was blindly `extend()`-ing the whole response array, which had already put one garbage row in the forecast ledger before this was caught. Fixed: each per-holding call now keeps only the entry whose `symbol` matches what was actually asked for, discarding anything else in the array.
